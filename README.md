...
- CVT (which has been out of business since the early 2000s) never produced any software versions for programming their keyboard, that ran on Windows versions later than XP, and hardly anyone runs XP or older OSes anymore;

- I have used Avant keyboards for decades, and occasionally they will lose their programming (reverting to the factory default key mappings and other settings), or else I would need to make some change to the keyboard programming;
...
1. Actually programming an Avant keyboard must be done on a PC that has a physical PS/2 keyboard port (a combo PS/2 keyboard/mouse port should work too).  It won't work on an Avant keyboard connected through a USB-to-PS/2 converter.
